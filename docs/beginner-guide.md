...
很多人第一次使用 skill 会有一个误解：以为把 CCFTS 装进 AI 后，AI 就会自动知道自己的企业、项目、岗位和本地口径。

实际上不会。

CCFTS 只提供施工企业财税领域的通用工作方法，例如快报怎么检查、增值税跨区域预缴怎么判断、质保金怎么复核。它不知道你所在企业的具体制度，也不知道你处理的是项目部、项目公司、分公司还是子公司，更不知道本次数据属于哪个期间、哪些资料已经脱敏、哪些结论不能用于正式报送。

所以正式使用时，需要同时提供两类资料：

1. **CCFTS skill**：告诉 AI 施工企业财税工作通常应该怎么做。
2. **项目背景包**：告诉 AI 这一次具体处理什么主体、什么期间、什么资料、什么边界。

你可以使用 [context-pack-template.md](context-pack-template.md) 填写背景包。同一个项目可以长期保留一份背景包，每次只更新期间、资料清单和本次任务。

简单说：

> Skill 解决“AI 懂不懂施工企业财税方法”的问题；背景包解决“AI 懂不懂你这次具体工作”的问题。
...
如果你只会用豆包、马维斯这类桌面 AI 助手，最简单的方式不是打开 97 个 skill 文件，而是使用 `agent-packs/`。
