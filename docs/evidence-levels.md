...
| **L2** | `practice-derived` | 基于施工企业实务经验归纳，需结合企业自身制度复核 | 操作类技能、教学 demo、脱敏样例 |
| **L3** | `demo-verified` | 已通过 `examples/` 中的脱敏 demo 验证流程正确性 | 3 个 demo 已有 input/expected 教学样例，但尚未建立自动 output 回归，**暂未达到此等级** |
| **L4** | `accountant-reviewed` | 已由持证 CPA / 税务师 / 企业资深财务审核并署名 | **0 / 97 文件**（全部为 `verified_by: pending`） |
| **L5** | `production-proven` | 已在真实业务流程中持续使用并验证 | **0 / 97 文件**（项目尚未在生产环境中部署使用） |

## 当前默认状态

- **所有 97 个文件**：`quality_tier: research-verified`，`verified_by: pending`
...
- 当前已有：公开脱敏教学 demo 和结构校验，仍需补充更多真实脱敏案例和专家复核记录。
...
### L4 → L5
- 在生产环境中实际使用 ≥ 3 个月
- 关键业务指标（如资料检查时间、错误率、增值税申报准确率）有量化改善
- 用户反馈系统建立并持续跟踪
