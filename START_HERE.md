...
3. [docs/user-context-template.md](docs/user-context-template.md)：填写你的使用者背景资料。

如果你只会用豆包、马维斯这类桌面 AI 助手，请优先使用 [agent-packs/](agent-packs/) 中的场景包，不要一开始就研究全部 97 个 skill。

## 从哪里开始（按你的角色选）

| 如果你是 | 从这里开始 |
|---------|-----------|
| 第一次听说、想先看看这是什么 | [README.md](README.md) |
| 非技术用户、不懂代码 | [docs/nontechnical-user-guide.md](docs/nontechnical-user-guide.md) |
| 财务、商务、成本、合约人员 | [docs/quick-start-10min.md](docs/quick-start-10min.md) |
| 想让 AI 更了解你的企业和项目 | 先填 [docs/user-context-template.md](docs/user-context-template.md) |
| 豆包、马维斯、手机端 AI 用户 | 打开 [agent-packs/](agent-packs/) 选对应场景包 |
| 想贡献脱敏案例或反馈试用结果 | 先看 [docs/feedback.md](docs/feedback.md)，再填 [docs/validation-log-template.md](docs/validation-log-template.md) |
| 想了解常见问题 | [docs/faq.md](docs/faq.md) |
| 开发者、MCP 配置 | [mcp/SMOKE_TEST.md](mcp/SMOKE_TEST.md)、[tests/](tests/)、[scripts/](scripts/) |
...
无论哪种方式，都建议同时提供 [使用者背景资料](docs/user-context-template.md)。skill 只告诉 AI 行业方法，背景资料才告诉 AI 你这次处理的具体主体、期间、资料和边界。熟悉后可以改用更完整的 [项目背景包](docs/context-pack-template.md)。
...
如果本次需要生成正式 Word/PDF、报告、请示、情况说明或汇报材料，最后还要加载 `ccfts-formal-word-output-format`，按中铁内部行文规范处理正文边界和排版要求。
```
